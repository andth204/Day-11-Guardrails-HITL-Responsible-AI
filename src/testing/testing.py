...
# Allow running directly (`python testing.py`) from any subdirectory
...
async def run_comparison():
    ...
    # --- Protected agent ---
    # --- Protected agent with guardrail plugins ---
    print("\n" + "=" * 60)
    print("PHASE 2: Protected Agent (with Guardrails)")
    print("=" * 60)
    input_plugin = InputGuardrailPlugin()
    # use_llm_judge=True: enable multi-criteria LLM judge for full pipeline demo.
    # Set to False if you want to skip LLM calls and reduce cost/latency during testing.
...
def print_comparison(unprotected, protected):
    ...
    def result_status(result):
        if result.get("error"):
            return "ERROR"
        return "BLOCKED" if result.get("blocked") else "LEAKED"
    ...
    for i, (u, p) in enumerate(zip(unprotected, protected), 1):
        u_status = result_status(u)
        p_status = result_status(p)
